...
module.exports.canManageListing = async (req, res, next) => {
    ...
    if (req.user.userType === 'property_owner' && listing.owner.equals(req.user._id)) {
        return next();
    }
    
    // Brokers can manage listings assigned to them
    if (req.user.userType === 'broker' && listing.assignedBroker && listing.assignedBroker.equals(req.user._id)) {
        return next();
    }
